...
import { ChatAttachment, CrossThreadTranscriptSource } from "@t3tools/contracts";
...
function toProjectionThreadMessage(
  ...
  return {
    ...
    ...(row.crossThreadSource !== null ? { crossThreadSource: row.crossThreadSource } : {}),
  };
}
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextCrossThreadSourceJson =
        row.crossThreadSource !== undefined ? JSON.stringify(row.crossThreadSource) : null;
      return sql`
        ...
        VALUES (
          ...
          COALESCE(
            ${nextCrossThreadSourceJson},
            (
              SELECT cross_thread_source_json
              FROM projection_thread_messages
              WHERE message_id = ${row.messageId}
            )
          ),
        ...
        DO UPDATE SET
          ...
          cross_thread_source_json = COALESCE(
            excluded.cross_thread_source_json,
